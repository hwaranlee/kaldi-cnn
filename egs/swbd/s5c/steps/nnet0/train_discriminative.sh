...
if [ ! -z $online_ivector_dir ]; then
  ...
  if [ $stage -le -8 ]; then
    ...
    echo $iters_per_epoch > $degs_dir/iters_per_epoch  || exit 1;
  else
...
#if [ ! $degs_done ] && [ -d $degs_dir/storage ]; then
if [ $stage -le -8 ]; then
  ...
  for x in $(seq $num_jobs_nnet); do
    for y in $(seq $nj); do
      utils/create_data_link.pl $degs_dir/degs_orig.$x.$y.ark
    done
    for z in $(seq 0 $[$iters_per_epoch-1]); do
      utils/create_data_link.pl $degs_dir/degs_tmp.$x.$z.ark
      utils/create_data_link.pl $degs_dir/degs.$x.$z.ark
    done
  done
fi
...
if [ $stage -le -6 ]; then
  ...
  for n in `seq 1 $num_jobs_nnet`; do
    egs_list="$egs_list ark:$degs_dir/degs_orig.$n.JOB.ark"
  done
...
if [ $stage -le -5 ]; then
  ...
  if [ $iters_per_epoch -eq 1 ]; then
    ...
    for n in `seq 1 $num_jobs_nnet`; do
      cat $degs_dir/degs_orig.$n.*.ark > $degs_dir/degs_tmp.$n.0.ark || exit 1;
      remove $degs_dir/degs_orig.$n.*.ark  # don't "|| exit 1", due to NFS bugs...
    done
  else # We'll have to split it up using nnet-copy-egs.
    egs_list=
    for n in `seq 0 $[$iters_per_epoch-1]`; do
      egs_list="$egs_list ark:$degs_dir/degs_tmp.JOB.$n.ark"
    done
    $cmd $io_opts JOB=1:$num_jobs_nnet $dir/log/split_egs.JOB.log \
      nnet-copy-egs-discriminative --srand=JOB \
        "ark:cat $degs_dir/degs_orig.JOB.*.ark|" $egs_list || exit 1;
    remove $degs_dir/degs_orig.*.*.ark
  fi
fi
...
if [ $stage -le -4 ]; then
  ...
  # Also, we should note that we used to do nnet-comine-egs-discriminative
  ...
  for n in `seq 0 $[$iters_per_epoch-1]`; do
    $cmd $io_opts JOB=1:$num_jobs_nnet $dir/log/shuffle.$n.JOB.log \
      ...
      ark:$degs_dir/degs_tmp.JOB.$n.ark ark:$degs_dir/degs.JOB.$n.ark || exit 1;
    remove $degs_dir/degs_tmp.*.$n.ark
  done
fi
...
#      nnet-am-average $nnets_list $dir/$[$x+1].mdl || exit 1;
    ...
    if [ $[$x%$iters_per_epoch] -eq 0 ]; then
        learning_rate=$(perl -e "print ($learning_rate*0.5);") 
	nnet-am-copy --learning-rate=$learning_rate $dir/$[$x+1].mdl $dir/temp.mdl
	mv $dir/temp.mdl $dir/$[$x+1].mdl
    fi
  fi
...
if $cleanup; then
  ...
  if [ -d $degs_dir ] && [ ! -L $degs_dir ]; then # only remove if directory is not a soft link.
