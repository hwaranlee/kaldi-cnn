...
# Filterbank exp:
#context_window=10
#splice_width=$context_window
#cat >$dir/splice_opts << EOF
#--left-context=$context_window --right-context=$context_window
#EOF
# Filterbank exp:
#cat >$dir/feat_dim << EOF
#21
#EOF
...
if [ $stage -le -2 ]; then
  ...
  stddev=`perl -e "print 1.0/sqrt($affine_in_dim*$affine_out_dim);"`
  conv_stddev1=0.01 #`perl -e "print 1.0/sqrt(40*6*1);"`
  conv_stddev2=0.01 #`perl -e "print 1.0/sqrt(1*5*2000);"`
  weight_decay=0.0005
...
ConvolutionComponent in-height=40 in-width=21 in-channel=1 kernel-height=40 kernel-width=6 stride=1 group=2000 out-height=1 out-width=16 learning-rate=$initial_learning_rate param-stddev=$conv_stddev1 bias-stddev=$bias_stddev weight-decay=$weight_decay
MaxpoolComponent in-height=1 in-width=16 in-channel=2000 pool-height-dim=1 pool-width-dim=2 pool-channel-dim=1
NormalizeComponent dim=16000
ConvolutionComponent in-height=1 in-width=8 in-channel=2000 kernel-height=1 kernel-width=5 stride=1 group=2000 out-height=1 out-width=4 learning-rate=$initial_learning_rate param-stddev=$conv_stddev2 bias-stddev=$bias_stddev weight-decay=$weight_decay
MaxpoolComponent in-height=1 in-width=4 in-channel=2000 pool-height-dim=1 pool-width-dim=2 pool-channel-dim=1
NormalizeComponent dim=4000
MaxpoolComponent in-height=1 in-width=1 in-channel=4000 pool-height-dim=1 pool-width-dim=1 pool-channel-dim=5
EOF

for n in `seq 1 $num_hidden_layers`; do #num_hidden_layers=2
(
echo AffineComponent input-dim=$affine_in_dim output-dim=$affine_out_dim learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev weight-decay=$weight_decay
echo MaxpoolComponent in-height=1 in-width=1 in-channel=$affine_out_dim pool-height-dim=1 pool-width-dim=1 pool-channel-dim=$[$affine_out_dim/$affine_in_dim]
echo NormalizeComponent dim=800
)>> $dir/nnet.config
done

(
echo AffineComponent input-dim=$affine_in_dim output-dim=$num_leaves learning-rate=$initial_learning_rate param-stddev=0 bias-stddev=0 weight-decay=$weight_decay
echo SoftmaxComponent dim=$num_leaves
) >> $dir/nnet.config
...
#MaxpoolComponent in-height=1 in-width=1 in-channel=$pnorm_input_dim pool-height-dim=1 pool-width-dim=1 pool-channel-dim=$[$pnorm_input_dim/$pnorm_output_dim]
#EOF
...
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    $cmd $dir/log/compute_prob_valid.$x.log \
      nnet-compute-prob --use-gpu=no $dir/$x.mdl ark:$egs_dir/valid_diagnostic.egs &
    $cmd $dir/log/compute_prob_train.$x.log \
      nnet-compute-prob --use-gpu=no $dir/$x.mdl ark:$egs_dir/train_diagnostic.egs
#    if [ $x -gt 0 ] && [ ! -f $dir/log/mix_up.$[$x-1].log ]; then
#      $cmd $dir/log/progress.$x.log \
#        nnet-show-progress --use-gpu=yes $dir/$[$x-1].mdl $dir/$x.mdl \
#          ark:$egs_dir/train_diagnostic.egs '&&' \
#        nnet-am-info $dir/$x.mdl &
#    fi
    
    train_prob=$(awk '(NF == 1)' < $dir/log/compute_prob_train.$x.log)
    #valid_prob=$(awk '(NF == 1)' < $dir/log/compute_prob_valid.$x.log)
    
    echo "Training neural net (pass $x) : train $train_prob "
