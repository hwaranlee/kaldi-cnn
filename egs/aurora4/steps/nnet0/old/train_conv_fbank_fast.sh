...
#in_height= $ext_feat_dim
#in_width= $[$splice_width *2 +1]
#in_channel=1
kernel_height=40
kernel_width=4
stride=1
...
--splice-width $splice_width --stage $get_egs_stage --cmd "$cmd" $egs_opts --io-opts "$io_opts" --transform-dir $transform_dir --feat_type "raw" \
...
#affine_input_dim=$[$out_height*$out_width *$group]
affine_input_dim=$[$out_height*$out_width *$group / $pool_h / $pool_w / $pool_c]


conv_stddev=`perl -e "print 1.0/sqrt($kernel_height * $kernel_width * $in_channel);"`
stddev=`perl -e "print 1.0/sqrt($pnorm_input_dim);"`
...
ConvolutionComponent in-height=$ext_feat_dim in-width=$in_width in-channel=$in_channel kernel-height=$kernel_height kernel-width=$kernel_width stride=$stride group=$group out-height=$out_height out-width=$out_width learning-rate=$initial_learning_rate param-stddev=$conv_stddev bias-stddev=$bias_stddev
...
TanhComponent dim=$affine_input_dim
AffineComponentPreconditionedOnline input-dim=$affine_input_dim output-dim=$pnorm_input_dim $online_preconditioning_opts learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
MaxoutComponent input-dim=$pnorm_input_dim output-dim=$pnorm_output_dim
...
#if [ $x -eq 1 ]; then
#mdl="nnet-init --srand=$x $dir/hidden1.config - | nnet-insert $dir/$x.mdl - - |"
#else
    ...
    $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
      ...
      ark:$egs_dir/egs.JOB.$[$x%$iters_per_epoch].ark ark:- \| \
       nnet-train$parallel_suffix$perturb_suffix $parallel_train_opts $perturb_opts \
        --minibatch-size=$this_minibatch_size --srand=$x --use-gpu=$gpu_id  "$mdl" \
        ark:- $dir/$[$x+1].JOB.mdl \
      || exit 1;
