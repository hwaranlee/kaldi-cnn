...
for n in `seq 1 $num_hidden_layers`; do
(
echo AffineComponentPreconditionedOnline input-dim=$pnorm_input_dim output-dim=$pnorm_output_dim $online_preconditioning_opts learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
echo MaxpoolComponent in-height=1 in-width=1 in-channel=$pnorm_output_dim pool-height-dim=1 pool-width-dim=1 pool-channel-dim=$[$pnorm_output_dim/$pnorm_input_dim]
echo NormalizeComponent dim=$pnorm_input_dim
)>> $dir/nnet.config
done

(
echo AffineComponentPreconditionedOnline input-dim=$pnorm_input_dim output-dim=$num_leaves $online_preconditioning_opts learning-rate=$initial_learning_rate param-stddev=0 bias-stddev=0
