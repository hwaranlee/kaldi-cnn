...
steps/nnet2/make_denlats.sh --cmd "$decode_cmd -l mem_free=1G,ram_free=1G" \
      --nj $nj --sub-split 1  --num-threads 1 --parallel-opts "-pe smp 2" \
      --transform-dir exp/tri4b_ali_si284 \
     data/train_si284 data/lang $dir "$dir"_denlats

steps/nnet2/align.sh  --cmd "$decode_cmd $gpu_opts" \
      --use-gpu no --transform-dir exp/tri4b_ali_si284 \
      --nj $nj data/train_si284 data/lang $dir "$dir"_ali
...
steps/nnet2/train_discriminative.sh --cmd "$decode_cmd" --learning-rate 0.00002 \
    ...
    --num-threads 1 --parallel-opts "$gpu_opts" --stage -3 data/train_si284 data/lang \
    "$dir"_ali "$dir"_denlats $dir/final.mdl "$dir"_mpe_gpu
