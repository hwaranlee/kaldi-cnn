...
int64 TrainNnetSimple(const NnetSimpleTrainerConfig &config,
  ...
  while (true) {
    ...
    for (i = 0; i < config.minibatches_per_phase; i++) {
      ...
      tot_logprob_this_phase += DoBackprop(*nnet, examples, &examples_formatted,
                                           nnet, &accuracy);
	  tot_accuracy_this_phase += accuracy;
    ...
    if (i != 0) {
      KALDI_LOG << "Training objective function (this phase) is "
                << (tot_logprob_this_phase / tot_weight_this_phase) 
				<< "Frame accuracy is "
				<< (tot_accuracy_this_phase / tot_weight_this_phase)*100
				<< " over "
  ...
  if (tot_weight == 0.0) {
    ...
    KALDI_LOG << "Did backprop on " << tot_weight
	          ...
	          << " average frame-accuracy per frame is "
              << (tot_accuracy / tot_weight)*100;
